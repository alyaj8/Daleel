...
let initHolder = initTour;

const EditTourV4 = ({ navigation, route }) => {
  const [activitiesMode, setActivitiesMode] = useState("idle"); // add, edit, idle
  // Form State

  const getData = () => {
    let localTour = route.params.data;
    const tourData = {
      ...localTour,
      // check if new Date() obj or firebase timestamp
      date: localTour.date.toDate ? localTour.date.toDate() : localTour.date,
      startTime: localTour.startTime.toDate
        ? localTour.startTime.toDate()
        : localTour.startTime,
  ...
  useEffect(() => {
    let localTour = getData();

    setTour(localTour);

    setActivities(localTour.activities);
    initHolder = localTour;
    // set form values
    // setValue("title", tourData.title);
    // setValue("city", tourData.city);
    // setValue("qty", tourData.qty);
    // setValue("meetingPoint", tourData.meetingPoint);
    // setValue("age", tourData.age);
    // setValue("description", tourData.description);
    // setValue("date", tourData.date);
    // setValue("startTime", tourData.startTime);
    // setValue("endTime", tourData.endTime);
    // setValue("status", tourData.status);
    // setValue("activitiesCustomizable", tourData.activitiesCustomizable);
    // setValue("activity", tourData.activity);
    // setValue("activities", tourData.activities);
    // setValue("price", tourData.price);
    // setValue("imageUrl", tourData.imageUrl);
  ...
  const enablePost =
    (isDirty && !!errors && Object.keys(errors).length === 0) || isUpdated;

  false && console.log("------------------------------------");
  false && console.log("🚀 ~ isDirty", isDirty);
  false && console.log("🚀 ~ errors", !!errors);
  false &&
